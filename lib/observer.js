define('observer',function(){
    ...
    var edit_chating = function(chatLI)
    {
        ...
        else if(chatMessage == 'chat-message-text')
        {
            ...
            for(i = 0 ; i < len ; i ++)
            {
                if(children.getAttribute('data-a-target') == chatMessage)
                {
                    chatMessageSpan.push(children);
                }
                else if(children.getAttribute('data-a-target') == emoteName)
                {
                    emoteSpan.push(children);
                }
                else if(
                    children.firstChild != null &&
                    children.firstChild.nodeName == 'SPAN' &&
                    children.firstChild.getAttribute('data-a-target') == emoteName)
                {
                    emoteSpan.push(children.firstChild);
                }
            }
